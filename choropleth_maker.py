...
Future Plans:
Purchase Dates and Amount.
Purchase Types to better spot increasing or decreasing trends.
Doctor Seen.
Map to be recreated each night with new sales data.
"""

# TODO Create folium map using data and package it up to be used by main flask app.

import folium
import pandas as pd

data = pd.read_csv("sample_data.csv")   # Pull out data
...
def average_dollar(money):  # Blue to Yellow to Orange; Using tags instead of heat mapping
    if int(money) < 50:
        ...
        return '#7cf03d'    # Yellow-Green
    elif int(money) < 300:
        return '#b5e238'    # Pale yellow
    elif int(money) < 350:
