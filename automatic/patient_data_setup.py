...
class Patient(object):
    ...
    def set_new_appointment(self):     # Will add new patients to appointment times. (Doesn't actually set appointment)
        patient_list = self.db.view('patients', field='id, last_purchase', conditional=('last_purchase', None))
        if len(patient_list) > 1:
            new_patients = random.sample(patient_list, 2)
        else:
            return False
        for person in new_patients:
            self.set_appointment(person)
